...
- AskUserQuestion으로 리뷰 결과를 PR 코멘트로 게시할지 질문한다:
  ...
  {
    "questions": [{
      "question": "리뷰 결과를 PR 코멘트로 게시할까요?",
      "header": "코멘트 게시",
      "options": [
        {"label": "게시", "description": "PR에 리뷰 코멘트를 게시"},
        {"label": "건너뛰기", "description": "터미널 출력만으로 종료"}
      ],
      "multiSelect": false
    }]
  }
  ```
- "게시" 선택 시 `gh pr review <PR번호> --comment --body "<리뷰 결과>"`로 게시한다
- "건너뛰기" 선택 시 터미널 출력만으로 종료한다
