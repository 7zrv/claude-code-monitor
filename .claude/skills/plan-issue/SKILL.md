...
### 5. 자체 검증
작성된 계획이 이슈 목표를 달성할 수 있는지 스스로 검증한다.

- **수치 목표 검증**: 이슈에 수치 목표가 있으면 (예: 커버리지 80%), 계획의 변경 범위가 수학적으로 목표를 달성할 수 있는지 계산한다
  - 예: 테스트 커버리지 이슈 → 테스트 가능 라인 / 전체 라인 ≥ 목표%
- **체크리스트 완전성**: 이슈 본문의 작업 내용 체크리스트가 모두 계획에 반영되었는지 확인한다
- **누락 검출**: 이슈 목표 달성에 필요하지만 계획에서 빠진 항목을 식별한다
- **자동 보정**: 검증에서 문제가 발견되면, 사용자에게 묻지 않고 즉시 계획을 수정하여 `PLAN.md`를 갱신한다
