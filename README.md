...
- 세션 export JSON에는 기본 요약/이벤트 외에도 파생 리스크와 연결된 alert 목록이 함께 포함됩니다.
...
최근 토큰 카드가 비정상적으로 크게 보이던 구버전에서 업그레이드한 경우, 기존 `~/.claude/monitor.db`의 시간 버킷이 이미 과대 적재돼 있을 수 있습니다.

한 번만 아래처럼 DB를 지우고 다시 시작하면 이후 버전부터는 같은 세션 로그를 재시작 때마다 중복 적재하지 않습니다.

```bash
rm ~/.claude/monitor.db
```
